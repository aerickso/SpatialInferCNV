...
Start by downloading a publicly available 10x Genomics Visium dataset [Breast Cancer](https://www.10xgenomics.com/resources/datasets/human-breast-cancer-block-a-section-1-1-standard-1-1-0).
...
Next, we will first demonstrate how to create an annotation .csv file from the Loupe Browser. The user will then read in a .csv file containing histological annotations and convert it to the R `data.frame` format. Histological annotations in our manuscript were performed at ‘per-spot’ level by clinical histopathologists who are expert trained to assess morphology of prostate samples. These detailed annotations permit clinical translation in a spatio-biological context. These annotations are primarily used in our analyses to enable groups of spots to be selected according to specific morphological classification (e.g. tumour versus benign; or epithelial versus stroma). We have included specific details of our histological annotation work-flow in the manuscript (Update with publication link upon publication).

It is important to emphasize, that for our manuscript, histological annotation work was not done in R, but done using the [10X Genomics LoupeBrowser](https://www.10xgenomics.com/products/loupe-browser). For this userguide, we used version 5.1.0.
...
Finally, export the annotations. Start by selecting the 3 white dots at the upper right, and select "Export Histology" from the dropdown menu.
...
SpatialInferCNV functions also work with 1K array ST data from the [original version of spatial transcriptomics](https://www.science.org/doi/10.1126/science.aaf2403). This is functionally similar to the above `ImportCountData()`, but instead ImportOriginalSTCountData imports count data in .tsv format, appends a section identifier to the barcodes, and returns an R `data.frame`.
...
In this chunk, the user will subselect 10 spots at random from the selected barcodes, append an annotation for these 10 spots, and then run `OriginalST_MergingCountAndAnnotationData()` to select count data only from these 10 spots. This function also passes a QC check. This function is similar to `MergingCountAndAnnotationData()` ran above on Visium Data
...
SpatialInferCNV provides functions to visualize the number of genes with an inferred copy number alteration (as presented in Figure 1 of our manuscript). The user needs to generate the 1k array, spot level inferCNV HMM results.

In order to run the next function, the user needs to run infercnv. InferCNV requires 3 inputs: a count file, an annotation file, and a gene loci file. We will output the first annotation and count file created above. Documentation to create a gene loci file can be found [here at the infercnv wiki documentation](https://github.com/broadinstitute/inferCNV/wiki/instructions-create-genome-position-file). To save you time, we provide a file download in the chunk. Author Note: given that the repo is private, this is currently commented out: [manually download](https://raw.githubusercontent.com/aerickso/SpatialInferCNV/main/FigureScripts/) the siCNV_GeneOrderFile.tsv file from the repo.
...
Having then run inferCNV and obtained HMM outputs, the user then needs to further format the data prior to using the functions for 1k array visualization
...
Having now obtained the necessary inputs, you can run `ExtractSectionWise()`.
...
***
Using the output from `ExtractSectionWise()`, you can then create a matrix for spatially plotting the 1k array data with `Output_PGA_Visualization_MatrixGreyNA()`.
...
***
Using the output from `Output_PGA_Visualization_MatrixGreyNA()`, we can finally spatially plot the 1k array data with `Plot_PGA_Visualization_Matrix()`.
...
The spots should be now visualized with the number of genes with an inferredCNV. Remember that we are using data from <10 spots, so the majority of the image should be grey.

![](./Images/BC23209_C1_PGA_SpatialVisualization_2022-03-27.png)

This ends the user guide, complementary information is embedded in the provided scripts.