...
# Downloading publicly available Visium Datasets (10x Genomics) and importing data with ImportCountData()
...
datasets](https://www.10xgenomics.com/resources/datasets?) including
datasets from the Visium Assay.
...
# Preparation of example histological annotations in the LoupeBrowser

We will first demonstrate how to create an annotation .csv file from the
LoupeBrowser, and then We will read in a .csv file containing
...
select/deselect certain groups for analysis. Specific details of how we
designed the histological annotation process can be found in our
manuscript (Update with publication link upon publication).
...
![](./Images/LB_Userguide_12.png) If this all was done properly, you
...
![](./Images/LB_ExportHistology.png) Select “Exclude Unlabeled”.
...
# Importing Histological Annotation Data from ImportHistologicalAnnotations().

We will next import the histological annotation .csv file containing
histological annotations and convert it to the R dataframe format. This
was created in the previous step, but we also provide this .csv file in
the directory if you wish to skip to this step.
...
# Merging histological annotation data with count data with MergingCountAndAnnotationData()

This next step demonstrates functionality of
MergingCountAndAnnotationData(). This function requires 3 parameters: a
section annotation string (this needs to be identical from the
annotation and count files), the annotation dataframe, and the count
dataframe)
...
# Selecting finalized annotations for export for use in infercnv::run with FinalAnnotations()
...
# Manual selection of output groups from a infercnv run plot for visualization and further analysis with SelectingSubTreeData()
...
output image. The output of SelectingSubTreeData() is a dataframe, with
barcodes in the first column, and the node number (identified above), in
the second column.
...
The MergedNodes dataframe object can be used, for example, to output a
new annotation file, to be imported into inferCNV, and then run inferCNV
with cluster_by_groups=TRUE, to output a plot, clustered by clones.
...
Open the LoupeBrowser file that was downloaded earlier. Select the 3
white dots at the right, and select “Import Categories…”
...
# Working with 1k array spatial array ST data and importing data with ImportOriginalSTCountData()
...
section identifier to the barcodes, and returns an R dataframe.
...
The above Mendeley data repository also provides the spot selection
file.
...
# Merging 1k array histological annotation data with count data with OriginalST_MergingCountAndAnnotationData()
...
# Extracting sectionwise, spot level HMM data with ExtractSectionWise()

ExtractSectionWise() is designed to extract specific section, spot level
HMM data: while not relevant for the userguide as we are using data from
only 1 section, this is useful in the modified output HMM dataframes
containing multiple sections data.
...
# Creating a visualization matrix for the HMM 1k array data with Output_PGA_Visualization_MatrixGreyNA()

Using the output from ExtractSectionWise(), we can then create a matrix
for spatially plotting the 1k array data with
...
# PlottiPlot_PGA_Visualization_Matrix()
