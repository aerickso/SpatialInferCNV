...
Download all the data from [Mendeley](https://data.mendeley.com/v1/datasets/svw96g68dv/draft), specifically all folders from: count_matrices/Patient 1/Visium_with_annotation/.
...
We then import the consensus pathology annotations and select benigns only, and created an annotation dataframe.
...
We use the function ImportCountData(), which requires a section label, and a path to the corresponding .h5 file. Again these are provided from the Mendeley repository (as described above).
...
Next, we merge annotations with count data to get section wise count matrices of only benign spots. This also applies a QC threshold (only allowing spots with 500 UMIs or more to pass to the filtered dataframes).
...
We then merge all the sectionwise dataframes together, replace joined NA's with 0's (inferCNV requires this), and output final count and annotation .tsv files that are required for infercnv:run.
...
The siCNV_GeneOrderFile.tsv has been provided here: https://github.com/aerickso/SpatialInferCNV/tree/main/FigureScripts.
...
The final file is provided at [Mendeley](https://data.mendeley.com/v1/datasets/svw96g68dv/draft): Count_matrices/Patient 1/Consensus Pathology.csv.